...
<xsl:when test="$targetPlatform = 'oc4j1013'">
@Stateless
@StatelessDeployment(localLocation="ejb/<xsl:value-of select="/PROCEDURES/PROCEDURE/PACKAGE_NAME_FIRSTCAP" />")
</xsl:when>
<xsl:otherwise>
</xsl:otherwise>
</xsl:choose>
public <xsl:if test="$abstract='true'">abstract</xsl:if> class <xsl:value-of select="/PROCEDURES/PROCEDURE/PACKAGE_NAME_FIRSTCAP" />_impl<xsl:if test="$abstract='true'">_abstract</xsl:if> implements <xsl:value-of select="/PROCEDURES/PROCEDURE/PACKAGE_NAME_FIRSTCAP" />
{

    private Log logger = LogFactory.getFactory().getInstance(getClass());

    public <xsl:value-of select="/PROCEDURES/PROCEDURE/PACKAGE_NAME_FIRSTCAP" />_impl<xsl:if test="$abstract='true'">_abstract</xsl:if>() {
    }  
