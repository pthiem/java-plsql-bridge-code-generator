...
public class PlsqlJavaBridgeExecutor {
	...
	public static PlsqlJavaBridgeExecutor getInstance() throws Exception {
		...
		if (instance == null) {
			InputStream stream = PlsqlJavaBridgeExecutor.class.getClassLoader()
					.getResourceAsStream("plsql_bridge.properties");
			if (stream != null) {
				props.load(stream);
				if (props.get("PlsqlJavaBridgeExecutorImplClass") != null) {
					instance = (PlsqlJavaBridgeExecutor) Class.forName(
							(String) props
									.get("PlsqlJavaBridgeExecutorImplClass"))
							.newInstance();
				}
			}
		}
	...
	public void execute(String packageName, String procedureName,
		...
		try {
			...
			if (xmlDocumentCache == null) {
				xmlDocumentCache = new HashMap();
			}
			;
			if (xmlArgumentCache == null) {
				xmlArgumentCache = new HashMap();
			}
			;
			...
			if (doc == null) {
				InputStream stream = this.getClass().getResourceAsStream(
						"/plsql_bridge/" + packageName
								+ "/package_metadata.xml");
			...
			String cacheKey = "procedureMetaData_" + packageName + "_"
					+ procedureName;
			Node procedureMetaData = (Node) xmlArgumentCache.get(cacheKey);
			if (procedureMetaData == null) {

				xpath = getXPath(xpath);
				expr = xpath.compile("//PROCEDURE[PROCEDURE_NAME='"
						+ procedureName.toUpperCase() + "']");
				procedureMetaData = (Node) expr.evaluate(doc,
						XPathConstants.NODE);
			...
			if (argCount == null) {
				...
				argCount = (Double) expr.evaluate(procedureMetaData,
			...
			plsqlStatementSQL = "{call " + packageName + "." + procedureName
					+ "(" + plsqlStatementSQL.substring(1) + ")}";
			...
			if (nodes == null) {
				nodes = (NodeList) xpath.compile("./ARGUMENTS/ARGUMENTS_ROW")
						.evaluate(procedureMetaData, XPathConstants.NODESET);
			...
			for (int i = 0; i < nodes.getLength(); i++) {
				...
				cacheKey = "nodes_input_inOut_" + packageName + "_"
						+ procedureName + "_" + i;
				String inOut = (String) xmlArgumentCache.get(cacheKey);
				if (inOut == null) {
					xpath = getXPath(xpath);
					inOut = (String) xpath.compile("./IN_OUT").evaluate(node,
							XPathConstants.STRING);
					xmlArgumentCache.put(cacheKey, inOut);
				}

				cacheKey = "nodes_input_javaGetMethodName_" + packageName + "_"
						+ procedureName + "_" + i;
				String javaGetMethodName = (String) xmlArgumentCache
						.get(cacheKey);
				if (javaGetMethodName == null) {
					xpath = getXPath(xpath);
					javaGetMethodName = "get"
							+ (String) xpath.compile("./ARGUMENT_FIRSTCAP")
									.evaluate(node, XPathConstants.STRING);
					xmlArgumentCache.put(cacheKey, javaGetMethodName);
				}

				cacheKey = "nodes_input_oraType_" + packageName + "_"
						+ procedureName + "_" + i;
				String oraType = (String) xmlArgumentCache.get(cacheKey);
				if (oraType == null) {
					xpath = getXPath(xpath);
					oraType = (String) xpath.compile("./DATA_TYPE").evaluate(
							node, XPathConstants.STRING);
					xmlArgumentCache.put(cacheKey, oraType);
				}

				cacheKey = "nodes_input_oraFieldName_" + packageName + "_"
						+ procedureName + "_" + i;
				String oraFieldName = (String) xmlArgumentCache.get(cacheKey);
				if (oraFieldName == null) {
					xpath = getXPath(xpath);
					oraFieldName = (String) xpath.compile("./ARGUMENT_NAME")
							.evaluate(node, XPathConstants.STRING);
					xmlArgumentCache.put(cacheKey, oraFieldName);
				}

				// String oraTypeScale = (String)
				// xpath.compile("./DATA_SCALE").evaluate(node,
				// XPathConstants.STRING);

				cacheKey = "nodes_input_javaFieldName_" + packageName + "_"
						+ procedureName + "_" + i;
				String javaFieldName = (String) xmlArgumentCache.get(cacheKey);
				if (javaFieldName == null) {
					xpath = getXPath(xpath);
					javaFieldName = (String) xpath.compile("./ARGUMENT_LOWER")
							.evaluate(node, XPathConstants.STRING);
					xmlArgumentCache.put(cacheKey, javaFieldName);
				}

				Class javaFieldType = arguments.getClass()
						.getField(javaFieldName).getType();
				...
				if (oraType.equals("PL/SQL RECORD")) {
					...
					// Not supported by jdbc. JDBC cannot do pl/sql record
					// types.
					throw new Exception("Type PL/SQL RECORD for " + packageName
							+ "." + procedureName + "." + oraFieldName
					...
					// Not supported by jdbc. JDBC cannot do pl/sql record
					// types.
					throw new Exception("Type TABLE and ARRAYS for "
							+ packageName + "." + procedureName + "."
							+ oraFieldName
							+ " is not supported by the PLSQL BRIDGE.");

				}

				if (inOut.equals("IN") || inOut.equals("IN/OUT")) {

					Object value = arguments.getClass()
							.getMethod(javaGetMethodName)
							.invoke(arguments, (Object[]) null);

					if (javaFieldType.equals(Long.class)) {

						stmt.setObject(i + 1, value, OracleTypes.NUMBER); // Set
																			// object,
																			// in
																			// case
																			// of
						...
						Timestamp timestamp = (value == null ? null
								: new java.sql.Timestamp(
										((java.util.Date) value).getTime()));
						stmt.setTimestamp(i + 1, timestamp); // Works with time
																// component
						...
						throw new Exception(
								"Java field type "
										...
										+ "."
										+ oraFieldName
										+ " is not supported by the PLSQL BRIDGE for *input*.");
					...
					if (javaFieldType.equals(Long.class)) {
						...
						throw new Exception(
								"Java field type "
										...
										+ "."
										+ oraFieldName
										+ " is not supported by the PLSQL BRIDGE for *output*.");
			...
			for (int i = 0; i < nodes.getLength(); i++) {
				...
				// inOut
				cacheKey = "nodes_output_inOut_" + packageName + "_"
						+ procedureName + "_" + i;
				String inOut = (String) xmlArgumentCache.get(cacheKey);
				if (inOut == null) {
					xpath = getXPath(xpath);
				...
				// javaSetMethodName
				cacheKey = "nodes_output_javaSetMethodName_" + packageName
						+ "_" + procedureName + "_" + i;
				String javaSetMethodName = (String) xmlArgumentCache
						.get(cacheKey);
				if (javaSetMethodName == null) {
					xpath = getXPath(xpath);
					javaSetMethodName = "set"
							+ (String) xpath.compile("./ARGUMENT_FIRSTCAP")
									.evaluate(node, XPathConstants.STRING);
				...
				// javaSetMethodName
				cacheKey = "nodes_output_javaFieldName_" + packageName + "_"
						+ procedureName + "_" + i;
				String javaFieldName = (String) xmlArgumentCache.get(cacheKey);
				if (javaFieldName == null) {
					xpath = getXPath(xpath);
				...
				// javaFieldTypedName
				cacheKey = "nodes_output_javaFieldTypedName_" + packageName
						+ "_" + procedureName + "_" + i;
				String javaFieldTypedName = (String) xmlArgumentCache
						.get(cacheKey);
				if (javaFieldTypedName == null) {
					xpath = getXPath(xpath);
					javaFieldTypedName = (String) xpath.compile(
							"./ARGUMENT_FIRSTCAP").evaluate(node,
							XPathConstants.STRING);
					xmlArgumentCache.put(cacheKey, javaFieldTypedName);
				}

				// String oraType = (String)
				// xpath.compile("./DATA_TYPE").evaluate(node,
				// XPathConstants.STRING);
				// String oraFieldName = (String)
				// xpath.compile("./ARGUMENT_NAME").evaluate(node,
				// XPathConstants.STRING);

				// isTypedRecord
				cacheKey = "nodes_output_isTypedRecord_" + packageName + "_"
						+ procedureName + "_" + i;
				Boolean isTypedRecord = (Boolean) xmlArgumentCache
						.get(cacheKey);
				if (isTypedRecord == null) {
					xpath = getXPath(xpath);
					isTypedRecord = (Boolean) xpath
							.compile(
									"count(./ARGUMENTS/ARGUMENTS_ROW[DATA_TYPE='PL/SQL RECORD' or DATA_TYPE='TABLE']) > 0")
				...
				if (inOut.equals("OUT") || inOut.equals("IN/OUT")) {
					...
					if (value == null) {
						...
						arguments
								.getClass()
								.getMethod(javaSetMethodName,
										new Class[] { javaFieldType })
								.invoke(arguments, new Object[] { value });
					...
					if (value != null) {
						...
						if (javaFieldType.equals(Long.class)) {
							...
							// get timestamp as stmt.getObject may get date only
							// component.
							...
							while (rs.next()) {
								...
								for (int colIndex = 1; colIndex <= rsmd
									...
									if (value2 != null) {
										...
										if (value2 instanceof java.util.Date) {
											...
											// interprest all dates as
											// timestamps, return date and time
											// components. the rs.getObject
											// method returns on the date
											// component.
											value2 = rs.getTimestamp(colIndex);

										} else if (value2 instanceof Number) {

											// No change required, could be
											// BigDecimal, Floats, Double,
											// (types of Number)
											// will be converted to type via
											// BeanUtils.copyProperties for
											// typed resultsets
											// Will be left unchanged for
											// untyped resultsets, client to
											// handle / convert from Number
											// type.
											...
											logger.debug("Unknown type for untyped result set for column "
													+ rsmd.getColumnName(colIndex)
													+ " "
													+ value2.getClass()
															.getName());
									...
									map.put(rsmd.getColumnName(colIndex)
											.toLowerCase(), value2);
								...
								if (!isTypedRecord) {
									...
									// Safe null handlilng behaviour for dates
									// like in beanutils v1.7.0 after upgrading
									// to bean utils v1.8.0
									BeanUtilsBean nullSafeBeanUtilsBean = new BeanUtilsBean();
									nullSafeBeanUtilsBean.getConvertUtils()
											.register(false, false, 0); // Peter
																		// JBOSS
																		// Added,
																		// http://stackoverflow.com/questions/2601175/how-to-ask-beanutils-to-ignore-null-values
									nullSafeBeanUtilsBean.copyProperties(
											typedRecord, map); // Use bean utils
																// to copy
																// properties
																// (easier than
																// doing
